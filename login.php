...
if (isset($_SESSION['user'])) {
    $user = [
        'isConnected' => true,
        'alias' => $_SESSION['user']['alias'], // Corrigé : minuscule car défini ainsi plus bas
        'isMage' => ($_SESSION['user']['role'] === 'Mage') || $_SESSION['user']['role'] === 'Admin',
        'balance' => [
            'gold' => $_SESSION['user']['gold'],     // Corrigé : minuscule
            'silver' => $_SESSION['user']['silver'], // Corrigé : minuscule
            'bronze' => $_SESSION['user']['bronze']  // Corrigé : minuscule
        ]
    ];
} else {
    $user = [
        ...
        'isMage' => false,
        'balance' => [
            'gold' => 0, // Corrigé pour uniformité
            'silver' => 0,
            'bronze' => 0
        ]
    ];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Corrigé : 'alias' en minuscule car ton input HTML a name="alias"
    ...
    if ($mode === 'register') {
        ...
        try {
            ...
            $foundUserRaw = $stmt->fetch(PDO::FETCH_ASSOC);
            flushProcedureResults($stmt);

            if ($foundUserRaw) {
                // 2. On force toutes les clés du tableau en minuscules
                $foundUser = array_change_key_case($foundUserRaw, CASE_LOWER);

                // 3. On utilise uniquement des minuscules maintenant
